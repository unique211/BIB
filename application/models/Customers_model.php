...
class Customers_model extends CI_Model
{
    ...
    function insert_api($table,$data){
        //check if available
        $ret = $this->db->where('customers_id', $data['customers_id'])->count_all_results($table);
        if($ret==0){
        //insert 
        ...
        $result = $this->db->insert_id();
            ...
            $result = $this->db->where('customers_id', $data['customers_id'])->select("id")->from($table)->get()->row();
            return $result->id;
        }
    }
