...
class Transactions extends CI_Controller
{
    ...
    public function save_trans()
    {
        ...
        $valid_extensions = array('jpeg', 'jpg', 'png', 'pdf' , 'doc');

        $ext = strtolower(pathinfo($image_name, PATHINFO_EXTENSION));

        $final_image = 'trn_'.rand(1,1000000).'_'.$image_name;

        if(in_array($ext, $valid_extensions)) 
        { 
            $bsurl=base_url();

            $path ='./userfiles/transacrionfile/'.strtolower($final_image); 
            ...
            if(move_uploaded_file($tmp,$path)) 
            {
                ...
                if ($this->transactions->addtrans($payer_id, $payer_name, $pay_acc, $date, $debit, $credit, $pay_type, $pay_cat, $paymethod, $note, $this->aauth->get_user()->id,$path)) {
                    echo json_encode(array('status' => 'Success', 'message' =>
        ...
        {
              echo json_encode(array('status' => 'Warning', 'message' =>
                        'Selected File Type is not allow to upload , Please try again !'));
        }
    ...
    public function save_transfer()
    {
       if ($this->aauth->get_user()->roleid < 3) {
        ...
        $pay_acc = $this->input->post('pay_acc');   //Teamleader Id
        $pay_acc2 = $this->input->post('pay_acc2'); //Salesperson Id 
        $amount = $this->input->post('amount');

        if($pay_acc != '' || $pay_acc2  != '' || $amount < 0 || $amount !='' || $pay_acc !=null || $pay_acc2 !=null || $amount !=null)
        {
            $check=$this->shows->checkbalancebeforetransfer($pay_acc2,$amount);

               if($check)
               {
                    if ($this->transactions->addtransfer($pay_acc2,$pay_acc, $amount, $this->aauth->get_user()->id)) {
                    echo json_encode(array('status' => 'Success', 'message' =>
               ...
               else
               {
                    ...
                    echo json_encode(array('status' => 'Error', 'message' =>
                        'Selected Salesperson has not enought balance in their account to transfer !' )); 
               }
        }
        else
        {
             echo json_encode(array('status' => 'Warning', 'message' =>
                        'It appears you have forgotten to complete something!' )); 
        }
