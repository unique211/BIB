...

{
    ...
    //Set task status for show

    public function set_show_task()

    {
        ...
        $this->tools->setshowtask($id, $stat);
    ...
    public function view_task_show()
    {

         $id = $this->input->post('tid');
         $task = $this->tools->viewtaskshow($id);


          echo json_encode(array('name' => $task['showtaskname'], 'description' => $task['showtaskdescription'], 'employee' => $task['emp'], 'assign' => $task['assign'], 'priority' => $task['showpriority']));
    }
...

    {
     ...
     //Delete Task for Show

    public function delete_i_show()

    {
        ...
        if ($this->tools->deleteshowtask($id)) {
